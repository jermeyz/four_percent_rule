...
@st.cache_data
def build_data_set(principal,future_percent_return,periods,withdrawl_rate,rate_of_inflation):
    _init_amount = principal
    _periods = periods
    _future_percent_return = future_percent_return
    _withdrawl_rate = withdrawl_rate
    df = pd.DataFrame(index=range(_periods ), columns=[ 'principal','interest_earned','withdrawl_amount'])
    df.at[0,'principal'] = _init_amount
    df.at[0,'interest_earned'] = _init_amount * _future_percent_return
    df.at[0,'withdrawl_amount'] = (df.at[0,'interest_earned'] +  _init_amount) * _withdrawl_rate
    last_withdrawal_percent = _withdrawl_rate
    ...
    return df
def build_data_set_actual_data(principal,withdrawl_rate,real_data):
    init_amount = principal
    steps = 30
    withdrawl_percent = withdrawl_rate
    df = pd.DataFrame(index=range(steps ), columns=[ 'principal','interest_earned','withdrawl_amount','yearly_returns','inflation_rate'])
    df.at[0,'principal'] = init_amount
    df.at[0,'interest_earned'] = init_amount * (real_data.at[0,"sp_returns"] /100)
...
real_data = build_data_set_actual_data(principal,percent_withdrawl/100,rdf)
styled_dataset = real_data.style.format({'principal': '${:,.2f}','interest_earned': '${:,.2f}','withdrawl_amount': '${:,.2f}'})

with st.container(border=True):
    st.write(f"**Total interest earned REAL:** ${real_data['interest_earned'].sum():,.2f}")
    st.write(f"**Total amount withdrawn REAL:** ${real_data['withdrawl_amount'].sum():,.2f}")
